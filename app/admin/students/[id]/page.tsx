...
export default function StudentProfilePage() {
  ...
  const handleDocumentStatus = async (docId: string, status: "verified" | "rejected" | "pending") => {
    try {
                     ...
                     <div key={doc.id} className="p-4 rounded-lg flex flex-col md:flex-row md:items-center justify-between gap-4" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
                       <div>
                         ...
                         <span className="text-[10px] px-2 py-0.5 rounded uppercase tracking-wider font-bold" style={{ 
                           background: doc.status === "verified" ? "rgba(34,197,94,0.15)" : doc.status === "rejected" ? "rgba(239,68,68,0.15)" : "rgba(229, 168, 0,0.15)", 
                           color: doc.status === "verified" ? "#86efac" : doc.status === "rejected" ? "#fca5a5" : "#E5A800" 
                         }}>
                           {doc.status}
                         </span>
                       </div>
